# Windows counterpart to ripgrep/install.zsh.

function phase_1_machine_installs {
    if (-not (Verify-Elevated)) {
        Write-Host "Skipping ripgrep machine install: this PowerShell process is not elevated." -ForegroundColor Yellow
        return
    }
...
function phase_3_dotfiles {
    link_file (Join-Path $PSScriptRoot "profile.ps1") `
        (Join-Path $profileFragmentsDir "profile_ripgrep.ps1")
}
